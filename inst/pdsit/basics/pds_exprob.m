...
function [P MIN]= pds_exprob(X,T='lines')
%
...
%  [P MIN] = pds_exprob(X);			% 
%  [P MIN] = pds_exprob(X,T);		% T can be equal to 'lines' or 'columns'.
%  
%
%  Input:
...
%  [P MIN]= pds_exprob(X);
%
%  Thus, P is a 3D matrix. So that the probability of Pr(x_1=a,x_2=b,x_3=c)
%  is equal to P(a-MIN(1)+1,b-MIN(2)+1,c-MIN(2)+1)
%
...
%  Code developer: Fernando Pujaico Rivera <fernando.pujaico.rivera@gmail.com>
	...
	%%% creando histograma multidimensional.
	X=X-MIN'+1;
	for II=1:L
		x_vector=X(:,II)';
		id=vec2ind(P_SIZE,x_vector);
		P(id)=P(id)+1;
	end
